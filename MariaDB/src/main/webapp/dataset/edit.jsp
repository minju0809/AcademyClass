...
<%
DatasetVO m = (DatasetVO) request.getAttribute("m");
	...
	<div align=center>
		...
		<form action="<%=path %>/ShoppingController">
			...
			<table border="1" width=500>
				...
				<tr> 
			  	<td>회원번호</td> 
			  	<td><input type=text id=custno name=custno value="<%=m.getIdx() %>" /></td>
				</tr>
	     	<tr>
			    <td>센터명</td> 
			    <td><input type=text name=custname value="<%=m.getCnterNm() %>" /></td>
				</tr>
				<tr> 
					<td>센터장</td>
		  		<td><input type=text name=phone value="<%=m.getCnterChNm() %>"  /></td>
	  		</tr>
	  		<tr> 
					<td>경도</td>
		  		<td><input type=text name=phone value="<%=m.getLat() %>"  /></td>
	  		</tr>
	  		<tr> 
					<td>위도</td>
		  		<td><input type=text name=phone value="<%=m.getLot() %>"  /></td>
	  		</tr>
	  		<tr> 
					<td>홈페이지</td>
		  		<td><input type=text name=phone value="<%=m.getHmpgAddr() %>"  /></td>
	  		</tr>
        <tr> 
        	<td>주소</td>
	        <td><input type=text name=grade value="<%=m.getRoadNmAddr() %>" /></td>
        </tr>
				<tr> 
...
var mapCenter = new kakao.maps.LatLng(<%=m.getLat() %> , <%=m.getLot() %>), // 지도의 중심좌표
    mapOption = {
